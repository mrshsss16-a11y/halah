...
async function categoryHandler(body, env, request, requestId, context) {
  ...
  try {
    const out = await applyProductCategory(env, { merchantId, productId, type });
    return {
      ok: true,
      ...out,
      message: out.replaced
        ? `تم — التصنيف صار «${out.categoryName}» بدل «${out.replaced}».`
        : `تم — أُضيف تصنيف «${out.categoryName}».`
    };
  } catch (err) {
    const status = Number(err?.status) || 0;
    if (err?.code === "CATEGORY_NOT_FOUND") return { ok: false, error: err.message, code: err.code };
    if (status === 403) {
