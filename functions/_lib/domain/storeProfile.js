...
function invalid(message, internal) {
  return new DomainError(400, message, "PROFILE_INVALID", internal);
}
...
function requireDb(env) {
  if (!env?.DB) {
    throw new DomainError(503, "الخدمة غير متاحة حالياً.", "DB_UNAVAILABLE", "storeProfile: DB binding missing");
  }
  return env.DB;
}
