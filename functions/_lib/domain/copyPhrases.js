...
const ADJ = "(?:[أا]نيق|فاخر|فخم|مريح|متين|مثالي|جميل|جمالي|فريد|مميز|عصري|جذاب|رائع|راق(?:ي)?)(?:ة|ه)?(?:اً|ًا|ا|ً)?";
...
const PATTERNS = [
  ...
  // «يعطيها مظهرًا فخمًا يناسب المناسبات» ⇒ «يناسب المناسبات» — الصفة وسط الجملة لا آخرها (2026-09-12 19:48)
  new RegExp(`\\s*(?:و)?(?:ت|ي)(?:عطي|منح|ضفي|ضيف)(?:ها|ه|ك)?\\s+(?:مظهر(?:اً|ًا|ا)?|إطلالة|اطلالة|لمسة|طابع(?:اً|ًا|ا)?)(?:\\s+(?:و)?${ADJ}){1,2}(?:\\s+(?:في\\s+نفس\\s+الوقت|بنفس\\s+الوقت|في\\s+الوقت\\s+نفسه))?(?![\\p{L}])`, "gu"),
  // «تبرز جمال إطلالتك» — الفعل والحكم ومفعوله معاً، لا «…عريضة إطلالتك» (تنورة 2026-09-12 21:19)
  new RegExp(`\\s*،?\\s*(?:و)?(?:ت|ي)(?:زيد|ضيف|برز|عزز)\\s+(?:من\\s+|إلى\\s+|الى\\s+)?${OWN}\\s+(?:[إا]طلالت(?:ك|ها|ه)|مظهر(?:ك|ها|ه))(?![\\p{L}])`, "gu"),
  // «للطلعات اللي تحتاج أناقة» ⇒ «للطلعات» — بقاؤها كان يحذف جملة التنسيق كلها
