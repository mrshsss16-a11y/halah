...
const CODE_LESSONS = {
  ...
  TOO_SHORT: ["وصف قصير رغم وجود صورة", "فقرتان أو ثلاث: القطعة، باقي تفاصيلها المرئية، قيم الخيارات الواردة وحدها — بلا مناسبة لم يذكرها التاجر ولا جدول مقاسات"],
...
export function detectLessons({ draft = "", codes = [], final = "", rawNotes = "", notes = "", sourceText = "" } = {}) {
  ...
  if (cta) add("SALES_CTA", `«${cta[0]}…» دعوة بيع داخل الوصف`, "وصف القطعة فقط، بلا دعوة شراء", "writer");
  ...
  if (/(?<!\p{L})(?:ال)?طابع\s+العام(?!\p{L})/u.test(d)) add("NOTE_LABEL", "نسخ عنوان الملاحظات «الطابع العام» بالوصف", "احذفي السطر؛ المناسبة تُكتب من بيانات التاجر وحدها لا من الصورة", "writer");
