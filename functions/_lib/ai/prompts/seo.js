...
` +
  ...
  const visionOutputRule = visionNotes
    ? `\n10. **الوصف والنبذة يجب أن يذكرا صراحةً** اللون ونوع القصّة (كما وردا بملاحظات صورة المنتج أعلاه)، مع اقتراح استخدام معقول (مناسبة/موسم/إطلالة) مشتق من هذي المرئيات وحدها. تجاهل هذي التفاصيل مع توفرها = وصف ناقص مرفوض.`
    : "";
  ...
  const taxonomyOutputRule = visionNotes && taxonomyBlock
    ? `\n11. **التزمي بنفس مصطلحات ملاحظات الصورة حرفياً** عند تسمية القصّة والياقة والأكمام — لا مرادفات ولا إعادة صياغة. اتساق التسمية عبر منتجات المتجر مقصود.`
    : "";
