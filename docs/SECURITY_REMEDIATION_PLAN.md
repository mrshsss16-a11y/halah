...
## ✅ الدفعة ١ — أوقف النزيف الحرج — **تمت** (2026-09-05، `124405e`، منشورة ومتحقَّقة حياً)

> C1: Zid POST=405 (المعالج محذوف) · C2: معرّفات وهمية تتقاسم حصة default-store (248→247→246
> لا 299 لكلٍ) · C4: status بجسم فارغ={linked:false} بلا معرّف · C3: esc منشورة + ٥ اختبارات
> جديدة (38/38). ملاحظة: طبقة rate-limit ناعمة بسبب سباق KV — تُصلّب بالدفعة ٤/H8.

### تفصيل الدفعة ١ *(للمرجع)*
...
| بند | إصلاح | ملف |
|-----|-------|-----|
| **H5** تصعيد أدمن | ~~عمود `is_admin`~~ — **قرار محسوم 2026-09-08: لا عمود دور بقاعدة البيانات** (قاعدة معمارية بـ`AGENT.md` §٧ و`PARALLEL_TRACKS.md`: لا مسار لمنح أدمن بالكتابة للـDB). الإغلاق بطبقتين: (١) رفض إيميلات `ADMIN_EMAILS` بالتسجيل ✅ `7fbaee3` · (٢) تحقق ملكية البريد ⬜ (P38 طبقة ٢، المرحلة ٣ بـ`COMPLETION_PATH.md`) | `signup.js` |
