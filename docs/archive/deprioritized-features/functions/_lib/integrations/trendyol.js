...
export async function tyFetch(conn, path, opts = {}) {
  if (conn?.isSimulation || conn?.environment === "simulation" || conn?.api_key === "simulated" || conn?.api_key === "simulation") {
    return { ok: true, isSimulation: true, simulatedPath: path };
  }
...
/** Fetch pending customer questions on Trendyol marketplace. */
export async function listCustomerQuestions(conn, { status = "WAITING_FOR_ANSWER", page = 0, size = 20 } = {}) {
  return tyFetch(conn, `/questions/sellers/${conn.seller_id}/questions/filter?status=${status}&page=${page}&size=${size}`);
}

/** Post an AI-generated response to a customer question on Trendyol. */
export async function answerCustomerQuestion(conn, questionId, text) {
  return tyFetch(conn, `/questions/sellers/${conn.seller_id}/questions/${questionId}/answers`, {
    method: "POST",
