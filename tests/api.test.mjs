...
async function runTests() {
  ...
  // ── تجربة أول تاجر — مرصود بثلاثة وكلاء وأُصلح (2026-09-09) ──────────
  {
    ...
    assert(
      /هذا المنتج بلا معرّف سلة/.test(dash) && /box\.classList\.remove\('hidden'\)/.test(dash),
      "UX-6: منتج بلا productId ⇒ صندوق النشر ظاهر بسبب صريح"
    );
    // #10 زر الجملة معطّل بلا منتجات.
    assert(
      /function setBulkGenerateEnabled/.test(dash) && /setBulkGenerateEnabled\(lastCatalogTotal > 0\)/.test(dash),
    ...
    assert(
      /خطأ خادم بلا حقل error كان يُعرض "تم الحفظ ✅"/.test(dash) && /if \(!res\.ok \|\| data\?\.error\) \{\s*showMsg\('agentFeedback'/.test(dash),
