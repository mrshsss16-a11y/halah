...
main()
  ...
  // ── خيارات المنتج (ألوان/مقاسات) بيانات مؤكَّدة فوق الصورة (2026-09-10) ──
  {
    ...
    assert(
      // 2026-09-15: سطر «لون الصورة لون النموذج المعروض» زرع «يظهر التصميم… في النموذج المعروض» بوصف حقيقي —
      // صار «صفي المنتج لا الصورة» مع المدى المتاح كله.
      /الخيارات هي المرجع/.test(withVars) && /صفي المنتج لا الصورة/.test(withVars) && /فاذكري المدى المتاح كله/.test(withVars) && !/لون الصورة لون النموذج المعروض/.test(withVars),
      "VAR-2: عند التعارض الخيارات تتقدّم على لون الصورة، والبرومبت لا يدفع الكاتب لذكر «النموذج المعروض»"
    );
    assert(
