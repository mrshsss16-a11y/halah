...
async function main() {
  ...
  // 2026-09-16: معيار C8 (٧.٤) أضاف إعادة ترتيب النبذة حين تكرر جملتها الأولى افتتاحية الوصف — القيمة
  // القديمة هنا كانت تنشر الجملة المكررة أولاً؛ الآن تتقدّم معلومة المدى (لم تذكرها الجملة الأولى) وتتبعها
  // الجملة المكررة بدل حذفها، فلا يضيع سياق اللون والقصّة.
  assert(page.copywriting.excerpt === "متوفرة بمقاسات من 36 (XS) إلى 44 (XL). تنورة سوداء ميدي بقصّة واسعة، بكسرات عريضة وقماش لامع.", `PC-1: النبذة تتقدّم بمعلومة المدى الجديدة قبل الجملة المكررة لافتتاحية الوصف («${page.copywriting.excerpt}»)`);
