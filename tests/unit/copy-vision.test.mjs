...
async function main() {
  // ── تحليل الصورة: "وفق الصورة" لا اختلاق (بلاغ 2026-09-07) ────────────────
  {
    ...
    assert(
      /ليس مصدراً للمناسبة/.test(withVision) && /تُكتب فقط إن وردت ببيانات التاجر/.test(withVision) && !/اقتراح استخدام معقول/.test(withVision),
      "VIS-13: المناسبة والاستخدام من بيانات التاجر وحدها لا مشتقة من الصورة (معيار ٦.٢، 2026-09-15)"
    );
    assert(
