...
- `verifyWaSignature` fail-closed فعلاً (`whatsapp.js:38`: `if (!appSecret) return false`).
- رقم التذكرة موحّد — استدعاء واحد فقط لـ`ticket_code` بـ`db.js`، لا حساب مكرر بالويبهوك.
...
- **"٦ مسارات بلا حصة" (البند الشهير بالتوثيق السابق) — تحقُّق 2026-09-06 يُبطله شبه كلياً:**
  ...
  `whatsapp/webhook.js` **مُقاس فعلاً** (`checkAndConsumeMonthly` بسطر ٨٧) — التعليق بجانبه
  يشهد أنه أُصلح سابقاً ("this path had zero metering **before**").
  **الفجوة الحقيقية الوحيدة الباقية:** `webhook.js:87` يلف الفحص بـ
  `.catch(() => ({ ok: true }))` — عطل D1/KV مؤقت يعني **رسائل غير محسوبة**، لا فشل
  مرئي (fail-open، لا fail-closed). قرار تجاري (يبقي البوت يرد أثناء عطل قاعدة
  البيانات بدل توقف كل التجار) — **لم يُحسم صراحة بعد**، يحتاج قرار مالك المشروع
  لا تعديلاً آلياً.
...
هذا القسم يفوت التحديث بسهولة بعد إصلاحات جانبية. تحقّق 2026-09-06 وجد ٣ بنود "دَين"
كانت مُصلَحة فعلاً منذ فترة بلا تحديث هذا الملف.
